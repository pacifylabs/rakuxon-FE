...
import { formatLocation, formatMoney } from '@/lib/catalogue/format';
...
import { CourseBrowser } from '@/app/universities/[slug]/CourseBrowser';
...
/**
 * Where this renders decides how "Proceed to apply" behaves — a public
 * visitor has no session yet and has to go create one; a dashboard visitor
 * already has one, so applying should happen right here.
 */
export type InstitutionDetailApplyMode =
  | { kind: 'public' }
  | {
      kind: 'dashboard';
      /** e.g. `/dashboard/schools` — where this same page lives inside the app shell. */
      basePath: string;
    };
...
export interface InstitutionDetailProps {
  ...
  applyMode: InstitutionDetailApplyMode;
}

/**
 * A university's full profile: about, courses, requirements, neighbours.
 *
 * Shared between the public `/universities/[slug]` page and the dashboard's
 * `/dashboard/schools/[slug]` — the record and every fact about it are the
 * same regardless of who's looking, so this renders identically either way.
 * The only thing that differs is `applyMode`: what "Proceed to apply" and a
 * course card's apply button actually do, and whether links to other
 * institutions stay inside the dashboard or lead back out to the public site.
 */
export async function InstitutionDetail({ slug, query, applyMode }: InstitutionDetailProps) {
  ...
  /** Where this same page lives, in whichever context it's rendering. */
  const detailPath = (targetSlug: string) =>
    applyMode.kind === 'dashboard' ? `${applyMode.basePath}/${targetSlug}` : universityRoute(targetSlug);

  const listPath = applyMode.kind === 'dashboard' ? applyMode.basePath : ROUTES.universities;

  const institutionApplyHref =
    applyMode.kind === 'dashboard' ? '#courses' : applyHref({ university: institution.slug });
  ...
  const coursesHref = (next: { level?: string; discipline?: string; page?: number }) => {
    ...
    return `${detailPath(institution.slug)}${search ? `?${search}` : ''}#courses`;
  };
        ...
        <SectionBand tone="muted" labelledBy="university-courses-heading">
          ...
          */}
          {disciplines.length > 1 && (
            <form
              method="get"
              action={detailPath(institution.slug)}
          ...
          {courses.length > 0 ? (
            <CourseBrowser courses={courses} dashboardApply={applyMode.kind === 'dashboard'} />
          ) : (
      ...
      */}
      ...
      {neighbours.length > 0 && (
        <SectionBand labelledBy="university-nearby-heading">
          ...
          <p className="mt-6 text-sm text-text-muted">
            <a
              href={`${listPath}?country=${institution.countryCode}`}
              className="rounded-sm text-primary underline focus-visible:outline-none focus-visible:ring focus-visible:ring-offset-2"
            >
