...
function CoursesList() {
  ...
  const load = useCallback(async () => {
    try {
      const result = await client.listAdminCourses({
  ...
  async function runAction(row: AdminCourseSummary, action: 'publish' | 'suspend' | 'revert') {
    setPendingId(row.id);
    try {
      const updated =
        action === 'publish'
          ? await client.publishCourse(row.id)
          : action === 'suspend'
            ? await client.suspendCourse(row.id)
    ...
    { header: 'Status', cell: (row) => <PublishStatusBadge status={row.status} /> },
    {
      ...
      cell: (row) => (
        <div className="flex justify-end gap-3">
          <a
            href={`/dashboard/catalogue/courses/${row.id}`}
  ...
  return (
    <section aria-labelledby="courses-heading">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 id="courses-heading" className="font-heading text-3xl font-bold text-text">
            Courses
          </h1>
          <p className="mt-2 max-w-prose text-base text-text-muted">
            Publish, suspend, or revert to draft — or edit a record's own fields. A course only
            reaches the public catalogue when both it and its institution are published.
          </p>
        </div>
        {hasPermission('catalogue.publish') && (
          <Button variant="primary" size="md" onClick={() => window.location.assign('/dashboard/catalogue/courses/new')}>
            New course
          </Button>
        )}
      </div>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-4">
        ...
        <label className="ml-auto flex items-center gap-2 text-sm text-text-muted">
          <span className="sr-only">Search by title or institution</span>
          <input
            type="search"
            value={query}
      ...
      {items && (
        <div className="mt-6">
          <DataTable
            ...
            emptyState={
              <EmptyState icon={GraduationCap} title="No courses match this filter" description="Try a different status or search term." />
            }
          />
